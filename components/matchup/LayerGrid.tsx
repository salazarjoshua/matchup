...
const AddTile = ({ className, ...props }: ComponentPropsWithoutRef<'button'>) => (
  <button type="button" className={cn('flex min-w-0 flex-col gap-2 text-left', className)} {...props}>
    <span className="border-[1.5px] h-thumb rounded-control border-disabled text-muted grid place-items-center border-dashed font-mono text-[16px]">
      +
    </span>
    <span className="text-micro text-placeholder truncate font-mono">add</span>
  </button>
);

const DropTarget = ({ className, ...props }: ComponentPropsWithoutRef<'div'>) => (
